...
def load_and_match_bpm_mp3(mp3_path, target_bpm):
    ...
    padded = np.pad(y_stretched, (0, max(0, total_samples - len(y_stretched))))[:total_samples]
    return padded
...
def load_mp3_as_np_array(mp3_path):
    ...
    return np.pad(raw, (0, max(0, total_samples - len(raw))))[:total_samples]
...
def generate_ambient_drum_loop(bpm, duration_sec):
    ...
    return (kick + tom + noise)[:total_samples]
...
def process_video(input_mp4):
    ...
    piano_track = np.pad(piano_track, (0, max(0, total_samples - len(piano_track))))[:total_samples]
    ambient_drum = generate_ambient_drum_loop(bpm, duration)
    main_music = load_mp3_as_np_array(extracted_mp3)

    mixed = 0.4 * main_music + 0.4 * ambient_drum + 0.5 * piano_track

    # === makemp3.py 결과 mp3 포함 (BPM 맞춤) ===
    ...
    if mp3_candidates:
        ...
        user_track = load_and_match_bpm_mp3(latest_mp3, bpm)
        mixed += user_track
