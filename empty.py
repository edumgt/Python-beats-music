...
# 파일 불러오기
sound = AudioSegment.from_file("33.mp3", format="mp3")

# 무음이 아닌 구간(nonsilent)을 감지 (단위: 밀리초)
# silence_thresh: 음량 임계값 (기본 -50dBFS)
# min_silence_len: 무음으로 간주할 최소 길이 (여기선 300ms 이상이면 무음으로 간주)
nonsilent_ranges = detect_nonsilent(sound, min_silence_len=300, silence_thresh=-50)
...
output.export("33.mp3", format="mp3")
